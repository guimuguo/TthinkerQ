...
public:
    ...
    void init(ui _n, ui _key_cap, ui *_ids, ui *_keys) { //// _n = vertices, _key_cap = vertices - 1, _ids = peel_sequence, _keys = degree
        if(keys == nullptr) keys = new ui[n];
        if(pres == nullptr) pres = new ui[n];
        if(nexts == nullptr) nexts = new ui[n];
        if(heads == nullptr) heads = new ui[key_cap+1]; //// key_cap = n - 1
        assert(_key_cap <= key_cap);
        min_key = _key_cap; max_key = 0;
        for(ui i = 0;i <= _key_cap;i ++) heads[i] = n;
        
        for(ui i = 0; i < _n; i++) insert(_ids[i], _keys[i]); //// node_idx, degree of node
    }
    
    // insert (id, key) pair into the data structure
    void insert(ui id, ui key) { //// id: node_id, key: degree of node
