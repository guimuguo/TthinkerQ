...
public:
    ...
    void enqueue(vector<T>& val_vec) {
    	int vec_size = val_vec.size();
    	vector<NODE*> nodes(vec_size);
    	for(int i=0; i<vec_size; i++)
    	{
    		nodes[i] = new NODE;
    		nodes[i]->value = val_vec[i];
    	}
    	...
    	nodes[vec_size-1]->next = NULL;
    	tail->next.store(nodes[0], memory_order_relaxed); //atomic "store"
    	tail = nodes[vec_size-1];
    }
