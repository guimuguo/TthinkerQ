...
#pragma omp parallel for schedule(dynamic, 1) num_threads(num_compers)
	for(int i=0;i<mnum_of_vertices;i++)
	{
		int nlist_len, nnb_no;
		int tid = omp_get_thread_num();

		memcpy(pnb_list[tid], &mppadj_lists[i][1], sizeof(int)*mppadj_lists[i][0]);
		nlist_len = mppadj_lists[i][0];
		for(int j=1;j<=mppadj_lists[i][0];j++)
			pbflags[tid][mppadj_lists[i][j]] = true;

		for(int j=1;j<=mppadj_lists[i][0];j++)
		{
			nnb_no = mppadj_lists[i][j];
			for(int k=1;k<=mppadj_lists[nnb_no][0];k++)
			{
				if(!pbflags[tid][mppadj_lists[nnb_no][k]] && mppadj_lists[nnb_no][k]!=i)
				{
					pbflags[tid][mppadj_lists[nnb_no][k]] = true;
					pnb_list[tid][nlist_len++] = mppadj_lists[nnb_no][k];
				}
			}
		}
		if(nlist_len>1)
			qsort(pnb_list[tid], nlist_len, sizeof(int), comp_int); //2-hop-list are also sorted.
